...
/// the main answer ships without a `<sentinel-pins>` block.
...
/// a sales/performance answer ships without a `<sentinel-trend>` chart block.
...
pub fn voice_assistant_style_block() -> String {
    [
        ...
        "  • Do not emit <sentinel-trend> or <sentinel-pins> blocks unless the user explicitly asks for charts, a map, or a visual breakdown.",
...
fn tool_protocol_block() -> String {
    [
        ...
        "  • If you already see a <tool_results> block from a Sentinel router pre-dispatch, treat that as your first iteration's results — go straight to the final answer unless a clear gap remains.",
        ...
        "  2. One to three <sentinel-trend> chart blocks (when there is any numeric/financial/time-series/regional data — see GROWTH CHARTS).",
        "  3. The <sentinel-pins>…</sentinel-pins> map block on its own lines at the very END.",
        "Never put pins or charts BEFORE the prose. Never skip the prose. If low on token budget, shorten the prose — never drop it.",
        "",
        "THE ONLY TWO STRUCTURED CONSTRUCTS YOU MAY EMIT ARE <sentinel-trend> AND <sentinel-pins>. Never invent any other <sentinel-…> tag (no <sentinel-pend>, <sentinel-report>, <sentinel-prose>, etc.) and never write a literal placeholder like 'Write the prose section here'. Write the actual prose directly as plain markdown.",
        "",
